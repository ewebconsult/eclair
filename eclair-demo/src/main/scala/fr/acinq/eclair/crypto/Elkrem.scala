...
object Elkrem {
  ...
  case class Receiver(nodes: Map[Int, BinaryData], size: Int)

  object Receiver {
    def init(size: Int) = Receiver(Map.empty[Int, BinaryData], size)

    def addHash(receiver: Receiver, hash: BinaryData, index: Int): Receiver = {
      val map = (receiver.nodes + (index -> hash))
      val map1 = map -- allChildren(index, receiver.size)
      receiver.copy(nodes = map1)
    }
  ...
  def allChildren(pos: Int, size: Int, acc: Seq[Int] = Seq.empty[Int]): Seq[Int] = {
    if (pos >= size / 2)
      acc
    else {
      val (l, r) = childrenIndex(pos)
      l +: (r +: (acc ++ allChildren(l, size) ++ allChildren(r, size)))
    }
  }
}
